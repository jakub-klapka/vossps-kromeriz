...
/*  when reasearching and looking at wp-admin/includes/meta-boxes.php. There is no way that I can see that will allow us to
	...
	if( !empty( $_GET["post"] ) ) {
		...
		// if value == yes then remove link
		if( $thisJSDeleteMetaValue == "yes" && ( get_post_type( $currentJSPostID ) == "page" || get_post_type( $currentJSPostID ) == "post" || get_post_type( $currentJSPostID ) == "studium" ) ){
			//$postTypeIs = "";
...
function js_remove_delete_link_add_checkBox_to_screen_settings($current, $screen){
	...
	if ( isset( $_GET["post"] ) ){
		...
		if( in_array( $screen->id, array("post", "page", 'studium') ) && ( get_post_type( $currentJSPostID ) == "page" ||
			get_post_type( $currentJSPostID ) == "post" || get_post_type( $currentJSPostID ) == "studium"  ) && current_user_can("administrator") ){
